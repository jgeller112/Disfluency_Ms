...
We fit the same contrasts as accuracy for each parameter in the ex-Guassian model.
...
fit_c <- read_rds("https://osf.io/m4nv5/download")
...
a <- hypothesis(fit_c, "blur1 > 0")
b <- hypothesis(fit_c, "blur2 > 0")

c <- hypothesis(fit_c, "sigma_blur1 > 0")
d <- hypothesis(fit_c, "sigma_blur2 = 0")

e <- hypothesis(fit_c, "beta_blur1 > 0")
f <- hypothesis(fit_c, "beta_blur2 = 0")
...
```{r}
a <- hypothesis(fit_mem1 , "blurblur1 < 0")
b <- hypothesis(fit_mem1, "blurblur2 < 0")
...
A summary of the diffusion model results can be found in @tbl-diffexpt1b. High-blurred words had a lower drift rate than clear and low blurred words, *b* = `r a$hypothesis$Estimate`, 90% CrI \[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`.
