...
For our recognition memory data, we fit a Bayesian generalized linear multilevel model with a Bernoulli distribution and a probit link function. In its simplest form, Signal Detection Theory (SDT) models can be viewed as regressions with a probit link. To estimate the key SDT parameter \$d\^{\\prime}\$, we modeled participant responses (i.e., whether they responded "old" or "new") as a function of item status (actual "old" vs. "new") and blurring level, using a Bayesian hierarchical generalized linear model with a binomial distribution and probit link.

Traditional SDT analyses have been informative and efficient for binary accuracy data. However, these approaches lack the precision and power of mixed-effects models. For this reason, we adopt a Bayesian generalized linear mixed-effects modeling approach to SDT \[see \@zloteanu2024 for an excellent tutorial on Bayesian SDT models\]

When fitting a GLMM with a probit link, model estimates map directly onto SDT parameters. Specifically, the interaction between item status (old vs. new) and the variable of interest (e.g., blurring level) on participants' responses corresponds to \$d\^{\\prime}\$. The criterion (\$c\$) can also be derived from the model by examining the main effect of the variable of interest on response behavior.
...
blur_acc <- read.csv("data/cleaned_context_rt.csv") |>
    dplyr::filter(lex=="m")
...
acc_c <- read_rds("data/fit_acc_context.rds")

# get lowblur-c comparison
# acc_lb <- read_rds("https://osf.io/wt3ry/download")
acc_lb <- read_rds("data/fit_acc_context_lbc.rds")
```
...
fit_mem1 <- read_rds("data/c_blmm_sdt_hblb.rds")

# sd_lb <- read_rds("https://osf.io/4qp38/download")
sd_lb <- read_rds("data/blmm_sdt_lbc.rds")
...
load("data/acc_blmm_expnocontext.RData")

# fit_acc_lbc <- read_rds("https://osf.io/yhz4c/download")
fit_acc_lbc <- read_rds("data/noc_acc_lbc.Rds")
```
...
fit_exg1 <- read_rds("data/brms_rt_priorall_noc (1).rds")
...
fit_wiener <- read_rds("data/weiner_diff_noc.rds")
...
fit_mem_noc <- read_rds("data/c_blmm_sdt_noc_hblb.rds")

#get the lowblur vs. c contrast
# fit_mem_lbc <- read_rds("https://osf.io/tucn9/download")
fit_mem_lbc <- read_rds("data/c_blmm_sdt_noc_lbc.rds")
```
...
load("data/acc_blmm_sc.RData")

# fit_acc_sc_lb <- read_rds("https://osf.io/ehjxq/download")
fit_acc_sb_lb <- read_rds("data/sc_acc_lbc.rds")
```
...
fit_sc <- read_rds("data/blmm_rt_sc.rds")

# fit_sc_lc <- read_rds("https://osf.io/49bgx/download")
fit_sc_lc <- read_rds("data/brms_sc_nointerp_lbc (1).rds")
...
fit_sc_mem <- read_rds("data/wf_sc_sdt_blmm.rds")

# fit_sc_mem_lb <- read_rds("https://osf.io/c8bqh/download")
fit_sc_mem_lb <- read_rds("data/wf_sc_sdt_blmm_lb.rds")
```
