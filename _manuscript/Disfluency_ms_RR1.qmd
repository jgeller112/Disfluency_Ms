...
We modeled response times with an ex-Gaussian distribution[^3], allowing the Gaussian mean/location ($\mu$), the Gaussian standard deviation ($\sigma$) and the exponential scale ($\beta = 1/\lambda$) to vary by condition. Please note that when we refer to $\beta$ we are referring to \$\\beta/\\tau\$ (we will use $\beta/\tau$ to refer to the parameter). \*\*When fitting the ex-Gaussian distribution we use the identity link for $\mu$, and the log link for $\sigma$ and $\beta/\tau$.\*\*
