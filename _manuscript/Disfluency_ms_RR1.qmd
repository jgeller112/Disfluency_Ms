...
I**n the present experiments, we pursued two aims related to perceptual disfluency. The first was to examine the replicability of the perceptual disfluency effect. To maximize the likelihood of observing this effect, we employed a manipulation that has previously been shown to enhance memory: perceptual blurring. @rosner2015 demonstrated across several studies that varying degrees of blurring (high vs. low) differentially affect memory, with high levels of blurring enhancing memory. Following their paradigm, we presented participants with clear words (no blur), low-blurred words (5% Gaussian blur), and high-blurred words (15% Gaussian blur).\*\* As @geller2018 emphasized, however, not all disfluency manipulations are created equal. Different perceptual manipulations affect processing in distinct ways, making it critical to identify which manipulations reliably produce disfluency effects and at what stage of processing. By testing multiple levels of perceptual disfluency, our study offers a more nuanced account of how encoding processes shape memory outcomes.**

**The second, more pivotal aim was to expand the methodological toolkit for investigating perceptual disfluency during encoding. To this end, we applied the ex-Gaussian distribution, which allows RT distributions to be decomposed into parameters reflecting different stages of processing. This approach offers a richer perspective beyond what mean response times alone can reveal. The ex-Gaussian model is widely used in the word recognition field [@balota2008], and its parameters are both interpretable and straightforward to implement. By using a distributional approach, we aim to clarify the specific processing stages at which perceptual disfluency affects encoding, thereby providing a more mechanistic account of when disfluency enhances memory and when it does not.**
...
All models were fit in *R* (v.4.5.0) [@R] using the Stan modeling language [@grant2017] via the *brms* package [@brms]. We used maximal random-effects structures justified by the design [@barr2013].
...
[^2]: **This is supported by the creator of `brms`Paul Buerkner in this Github post: <https://github.com/paul-buerkner/brms/issues/311>. However, there is much debate on the use of BFs.**
...
[^3]: The parameterization used by *brms* does not match the standard formulation of the ex-Gaussian distribution. A custom script was implemented to recover the traditional parameters.
...
blur_acc_new <- read_csv("https://osf.io/xv5bd/download") |> 
  dplyr::filter(lex == "m") |>
...
```{r}
#| label: tbl-acc1A1B
#| tbl-cap: "Summary of the posterior distribution for accuracy (Experiment 1A and 1B)"
...
```{r}
#| label: tbl-ex1A1B
#| tbl-cap: "Posterior distribution estimates for ex-gaussian distribution (Experiment 1A and 1B)"
#| apa-note: "CrI: 90% for one-sided tests and 95% for two-sided tests against 0. Posterior probability indicates the proportion of the posterior distribution that falls on one side of zero (either positive or negative), representing the probability that the effect is greater than or less than zero."
# Create the flextable
...
A visualization of how blurring affected processing during word recognition can be seen in the quantile and delta plots in @fig-deltaquant1A1B A summary of the ex-Gaussion model can be found in @tbl-ex1A1B. Beginning with the μ parameter, there was greater shifting for high blurred words compared to clear and low blurred words, *b* = `r round(a$hypothesis$Estimate, 3)`, 90% CrI \[`r round(a$hypothesis$CI.Lower, 3)`, `r round(a$hypothesis$CI.Upper, 3)`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`. Low blurred compared to clear words showed greater shifting, *b* = `r round(b$hypothesis$Estimate, 3)`, 90% CrI \[`r round(b$hypothesis$CI.Lower, 3)`, `r round(b$hypothesis$CI.Upper, 3)`\], ER = `r process_ER_column(b$hypothesis$Evid.Ratio)`.
...
v_1$blur <- factor(v$blur, level = c("High blur", "Low blur", "Clear"))
...
p1_dp <- ggplot(v_chb, aes(x = mean_rt, y = diff)) +
  ...
  labs(
    title = "Clear - High blur",
    x = "",
    y = "Group differences",
    tag = "B"
  ) + 
ggtitle('Delta Plots: Experiment 1A')
```
...
ggsave("figures/expt1A-dprime.png", width = 19, height = 8, dpi = 300, bg="white")
```
...
v_2  <- v_2 |> 
mutate(blur = factor(blur, level = c("High blur", "Low blur", "Clear")), 
...
#| fig-cap: "Quantile plots for each condition in Exeriment 1A and 1B (A) and Delta plots depicting the magnitude of the effect for hypotheses of interest over time in for Experiments 1A (B) and 1B (C). Each dot represents the mean RT at the .1, .3, .5, .7 and .9 quantiles."
#| fig-width: 15
#| fig-height: 10
...
ggsave(filename = './figures/figure_quantile_deltaplots1A1B, width = 12, height = 12, bg="white")
...
Using @brysbaert2019 as heuristic, detecting a fully attenuated interaction with 80% power requires approximately 210 participants. To be conservative, and to ensure sufficient sensitivity, we doubled the sample size relative to Experiments 1A and 1B. We thus preregistered a sample size of 432[^4]. All participants were recruited through SONA and Prolific.

[^4]: @brysbaert2019 suggested a sample size of 210 participants to detect a fully attenuated interaction with 80% power. In similar sample size was used in previous studies @geller2021.
...
```{r}
#| label: tbl-acc2
#| tbl-cap: "Posterior distribution estimates for the accuracy model (Experiment 2)"
#| apa-note: "CrI: 90% for one-sided tests and 95% for two-sided tests against 0. Posterior probability indicates the proportion of the posterior distribution that falls on one side of zero (either positive or negative), representing the probability that the effect is greater than or less than zero"
