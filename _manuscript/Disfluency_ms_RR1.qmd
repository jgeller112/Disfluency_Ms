...
**If the metacognitive account [e.g., @alter200; @pieger2016] holds, and the effect arises primarily at a post-lexical stage, one would expect a lengthening of the distribution tail (increases in** $\tau$**) for blurred relative to clear words. Importantly, this perspective suggests that memory performance may not differ between high- and low-blurred words, given that perceptual disfluency is assumed to be largely subjective in nature.**
...
```{r}
#| label: tbl-acc1A1B
#| tbl-cap: "Posterior distribution estimates for accuracy (Experiment 1A and 1B)"
