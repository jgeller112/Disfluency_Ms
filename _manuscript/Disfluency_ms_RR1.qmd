...
Experiment 1A successfully replicated the pattern of results found in @rosner2015. Specifically, we found high blurred words had lower accuracy than clear and low blurred words, but had better memory. Adding to these results, we used the ex-Guassin model model to gain further insights into the mechanisms underlying the perceptual disfluency effect. Descriptively, high blurred words induced a more pronounced shift in the RT distribution ($\mu$) and exhibited a higher degree of skew ($\beta$) compared to clear and low blurred words. However, low blurred words did not differ compared to clear words on $\mu$ or $\beta$. These patterns can be clearly seen in our quantile delta plots in @fig-deltaquant1A1B.

While this pattern suggests that highly blurred words influence both early and late processing, whereas low-blurred words affect only early stages, there has been considerable debate about the use of the ex-Gaussian distribution for making inferences about latent cognitive processes [@fitousi2020; @matzke2009].
...
**We replicated all the findings of Experiment 1A in a design where levels of blurring were not reinstated at test. In the appendix we present the results from our DDM analysis and it largely corresponds to Experiment 1A.**
...
#| tbl-cap: "Posterior distribution summary for fixed effects in the ex-Gaussian model (Experiment 2)."
...
p1 <- ggplot(v_chb, aes(x = mean_rt, y = diff)) +
  ...
  geom_label_repel(
    ...
    seed = 42,
    box.padding = 0.5,
    label.size = 0,   # removes box border
    fill = NA         # removes box fill
  ) +
  labs(
