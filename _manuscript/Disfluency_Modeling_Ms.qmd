...
data <- data.frame(
  ...
  Contrast = c("High blur vs. Low blur/Clear", "Low blur vs. Clear",
               ...
               "High blur vs. Low blur/Clear", "Low blur vs. Clear"),
  Ex_Gaussian_Predictions = c(
    "$\\mu$: No difference<br>$\\tau$: Increase", "$\\mu$: No difference<br>$\\tau$: Increase",
    "$\\mu$: Increase<br>$\\tau$: No difference", "$\\mu$: Increase<br>$\\tau$: No difference",
    "$\\mu$: Increase<br>$\\tau$: Increase", "$\\mu$: Increase<br>$\\tau$: Increase"
  ),
  DDM_Predictions = c(
    "$v$: No difference<br>$T_{er}$: Increase", "$v$: No difference<br>$T_{er}$: Increase",
    "$v$: Increase<br>$T_{er}$: No difference", "$v$: Increase<br>$T_{er}$: No difference",
    "$v$: Increase<br>$T_{er}$: Increase", "$v$: No Increase<br>$T_{er}$: Increase"
  ),
  Quantile_Plots = c(
    "Late Difference",
    "Late Difference", 
  ...
  Recognition_Memory_Predictions = c(
...
This study complies with transparency and openness guidelines. The preregistered analysis plan for this experiment can be found here: https://osf.io/q3fjn. All raw and summary data, materials, and R scripts for pre-processing, analysis, and plotting for Experiments 1 can be found at https://osf.io/6sy7k/. All deviations and changes from the preregsitraion are noted herein.
...
```{r}
#| label: fig-blur
#| fig-cap: Clear (left), high blur (15% blur) (right), and low blur (10%) (left) examples 
...
```{r}
#| label: tbl-acc1a
#| tbl-cap: Posterior accuracy estimates in Experiment 1a for each hypothesis.
...
acc_exp1a <- tab %>% 
    ...
    select(Hypothesis, Estimate, SE, `90% CrI`,ER, Post.Prob)%>% tt() %>%
...
```{r}
#| label: tbl-exgauss1a
#| tbl-cap: "Posterior Ex-Guassian estimates for each hypothesis in Experiment 1a"
...
tab %>% 
  ...
  mutate(
        ...
        `90% CrI` = str_glue("[{CI.Lower}, {CI.Upper}]")) %>%
    rename("SE" = "Est.Error", "ER" = "Evid.Ratio") %>%
    select(Hypothesis, Parameter, Estimate, SE, `90% CrI`,ER, Post.Prob)%>% tt() %>%
    format_tt(escape=TRUE)
...
p <- ggplot(v, aes(x = q, y = MRT*1000, colour = blur, group=blur))+
  geom_line(size = 1) +
  geom_point(size = 3) +
  scale_colour_manual(values=met.brewer("Cassatt2", 3)) +  scale_y_continuous(breaks=seq(500,1600,100)) +
...
#| tbl-cap: "Posterior DDM estimates for each hypothesis in Experiment 1a"
...
```{r}
#| label: tbl-acc1b
#| tbl-cap: Posterior accuracy estimates in Experiment 1b for each hypothesis.
#| 
...
```{r}
#| label: tbl-exgauss1b
#| tbl-cap: "Posterior Ex-Guassian estimates for each hypotheiss in Experiment 1b"
#| 
# Compute hypothesis tests
...
#| tbl-cap: "Posterior DDM estimates for each hypothesis in Experiment 1b"
...
```{r}
#| label: tbl-acc2
#| tbl-cap: "Posterior accuracy estimates for each hypothesis in Experiment 2"
...
#| tbl-cap: "Posterior Ex-Guassian estimates for each hypothesis in Experiment 2"
...
tab <- bind_rows(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u) %>%
  ...
  rename("SE" = "Est.Error", "ER" = "Evid.Ratio") %>%
  select(Hypothesis, Parameter, Estimate, SE, `90% CrI`, ER, Post.Prob)
...
p1 <- ggplot(v, aes(x = q, y = MRT, colour=frequency)) +
  ...
  labs(title = "Quantile Analysis", x = "Quantiles", y = "Response latencies in ms", tag = "A")
...
geom_label_repel(data=v_chb, aes(y=diff, label=round(diff,0)), color="black", min.segment.length = 0, seed = 42, box.padding = 0.5)  +
  labs(title= "High Blue vs. Clear", x = "", y = "Frequency Effect", tag = "B")
...
p3 <- ggplot(v_hlb, aes(x = mean_rt, y = diff)) + 
  ...
  labs( title = "High Blur - Low Blur", x = "", y = "Group differences")
...
d= hypothesis(fit_sc_mem, "isold1:frequency1 < 0")
...
Similar to Experiments 1a and 1a, there was better memory recognition for high blurred words compared to clear words (see @fig-m2-emmeans), \$\\beta\$ = `r a$hypothesis$Estimate`, 95% Cr.I\[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r a$hypothesis$Evid.Ratio`, and low blurred words, \$b\$ = `r b$hypothesis$Estimate`, 95% Cr.I\[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r b$hypothesis$Evid.Ratio`. There was no recognition memory difference between clear and low blur words, \$\\beta\$ = `r c$hypothesis$Estimate`, 95% Cr.I\[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r c$hypothesis$Evid.Ratio`. Low frequency words were better recognized than high frequency words, $\beta$ = `r d$hypothesis$Estimate`, 95% Cr.I\[`r d$hypothesis$CI.Lower`, `r d$hypothesis$CI.Upper`\], ER = `r d$hypothesis$Evid.Ratio`. There was strong evidence for an interaction between high blurred words (vs. clear) and frequency, \$\\beta\$ = `r e$hypothesis$Estimate`, 95% Cr.I\[`r e$hypothesis$CI.Lower`, `r e$hypothesis$CI.Upper`\], ER = `r e$hypothesis$Evid.Ratio`, with better memory for high frequency-high blurred words, \$\\beta\$ = -0.14, 95% Cr.I\[-0.21, -0.07\]. There was some evidence of an interaction between blurring and frequency for high blurred words vs. low blurred words, \$\\beta\$ = `r f$hypothesis$Estimate`, 95% Cr.I\[`r f$hypothesis$CI.Lower`, `r f$hypothesis$CI.Upper`\], ER = `r f$hypothesis$Evid.Ratio`. Looking at this post hoc, High frequency-high blurred words were better recognized than low-frequency-blurred words, \$b\$ = 0.11, 95% Cr.I\[0.04, 0.18\]. There was also an interaction between frequency and low blurred vs. clear words, \$\\beta\$= `r g$hypothesis$Estimate`, 95% Cr.I\[`r g$hypothesis$CI.Lower`, `r g$hypothesis$CI.Upper`\], ER = `r c$hypothesis$Evid.Ratio`. This interaction arose from low-frequency clear words being better remembered than low-frequency low blurred words.
