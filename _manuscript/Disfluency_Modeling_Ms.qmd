...
All models were fit in R (v.4.4.2; \@R) using the Stan modeling language [@grant2017] via the **brms** package [@brms]. We used maximal random-effects structures justified by the design [@barr2013], including random intercepts for participants and items, and random slopes for blurring level.

Contrast coding (effects/ANOVA coding) was applied using the `hypr` package \[@hypr\] for accuracy and signal detection models. To obtain all relevant comparisons, models were fit twice: once comparing high blur to clear and high blur to low blur, and once comparing low blur to clear. For ex-Gaussian and diffusion models, the intercept was suppressed (i.e., no baseline condition) to simplify estimation and improve convergence [@Singmann2017].
...
We report posterior means and 90% credible intervals (CrIs). For marginal mean differences, we also report the posterior probability that $\delta \neq 0$. Hypotheses were considered supported if zero was excluded from the 90% CrIs and $P(\delta > 0)$ approached 1. Estimated marginal means were extracted using a combination of **emmeans** [@emmeans], **bayestestR** [@bayestestR], and **brms** [@brms].

Model quality was checked via prior/posterior predictive checks, $\hat{R}$, and ESS [@vehtari2021]. We used the Evidence Ratio (ER), which generalizes Bayes factors for directional hypotheses: ER \> 3 indicates moderate to strong evidence for the hypothesis; ER \< 0.3 supports the alternative; ER values between 0.3–3 are inconclusive. We also use ER to provide for support for null point hypothesis tests ($\delta = 0$)

#### Accuracy

Accuracy (coded as correct vs. incorrect) was modeled using a Bayesian logistic regression with a Bernoulli distribution. We specified a Cauchy(0, 0.35) prior for population-level effects, as recommended by \[\@kinoshita2023\].

#### Ex-Gaussian model

We modeled response times with an ex-Gaussian distribution, allowing the Gaussian mean/location ($\mu$), the Guassian standard deviation (\$\\sigma\$) and the exponential scale ($\beta = 1/\lambda$) to vary by condition. Please note that when we refer to $\beta$ we are referring to $\tau$. We applied weakly informative priors (e.g., \$N(0,10)\$) on $\mu$, $\sigma$, and $\beta$ coefficients, with default priors for other parameters. To visualize distributional shifts, we also generated delta plots.

#### Diffusion model

We fit a hierarchical Bayesian Wiener diffusion model [@vandekerckhove2011] with accuracy coding, estimating drift rate (information processing efficiency), boundary separation (response caution; fixed at 0.5), and non-decision time. Weakly informative priors were used.

#### Recognition memory

Recognition memory data were analyzed using a Bayesian generalized linear multilevel model (Bernoulli distribution with a probit link). Following Signal Detection Theory (SDT; [@green1966]), participant responses ("old" vs. "new") were modeled as a function of item status ("old" vs. "new") and blurring level. We specified a Cauchy(0, 0.35) prior for population-level effects.

Bayesian GLMMs provide a more precise and flexible approach than traditional SDT analyses. In this framework, the interaction between item status and a predictor of interest corresponds to *d′*, while the main effects reflect response criterion. For a detailed discussion of Bayesian SDT modeling, see \[@zloteanu2024\]. Note that our model parameterization reflects *–c* (i.e., reversed sign), though for visualization purposes we use the conventional parameterization, where positive values indicate more conservative responding and negative values indicate a more liberal bias.
