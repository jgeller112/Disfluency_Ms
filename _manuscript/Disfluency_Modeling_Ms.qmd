...
For accuracy, we constructed contrast codes for the Blur variable using the applying ANOVA-style (effects) coding. For the Blur variable, we defined two orthogonal contrasts to capture key comparisons of interest. Contrast 1 contrasted High Blur and Clear, with High Blur coded as .5 and Clear as -.5 (with Low Blur coded as 0). Comparison 2 contrasted High Blur and Low Blur, with High Blur coded as .5 and Low Blur as -.5 (with Clear coded as 0). These contrasts allowed us to estimate both main effects and interaction terms for High Blur versus each of the other two levels. To capture the remaining comparison between Low Blur and Clear, we fit a second model in which the contrast coding was re-centered.
...
```{r}
# get file from osf
acc_c<-read_rds("https://osf.io/g9v2s")
...
sdt_priors <- c(set_prior("normal(0,1)", class = "b"))
```
...
fit_mem1 <- read_rds("https://osf.io/4j8vp/download")

sd_lb <- read_rds("https://osf.io/3qutn/download")

```

```{r}
#HB > C
a <- hypothesis(fit_mem1 , "isold1:blurblur1 > 0")
b <- hypothesis(fit_mem1 , "isold1:blurblur2 > 0")
...
@fig-dprimeexp1a highlights high-blurred words were better remembered than clear words, $\beta$ = `r a$hypothesis$Estimate`, 90% CrI \[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`, and low-blurred words, $\beta$ = `r b$hypothesis$Estimate`, 90% CrI \[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r process_ER_column(b$hypothesis$Evid.Ratio)`. There was strong evidence for no difference in sensitivity between clear words and low blurred words, $\beta$ = `r c$hypothesis$Estimate`, 95% CrI \[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r process_ER_column(c$hypothesis$Evid.Ratio)`.
...
```{r}
a <- hypothesis(fit_mem1 , "blurblur2 < 0")
b <- hypothesis(sd_lb , "blur1 < 0")
c<-  hypothesis(fit_mem1 , "blurblur1 > 0")
...
Low blurred words had a bias towards more "old" responses compared to clear words, $\beta$ = `r b$hypothesis$Estimate`, 90% CrI\[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r process_ER_column(b$hypothesis$Evid.Ratio)`, and high blurred words, $\beta$ = `r a$hypothesis$Estimate`, 90% CrI\[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`. High blurred words showed a more liberal bias compared to clear words, $\beta$ = `r c$hypothesis$Estimate`, 90% CrI\[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r process_ER_column(c$hypothesis$Evid.Ratio)`.
...
# get file from osf
fit_acc_noc <- read_rds("https://osf.io/pbhv6/download")
...
a <- hypothesis(fit_acc_noc, "blurblur1 <  0") 

# Ensure rounding applies only to numeric columns
a$hypothesis <- a$hypothesis |>
  mutate(across(where(is.numeric), ~ round(.x, 3)))

b <- hypothesis(fit_acc_noc, "blurblur2 <  0")
...
A summary of posterior estimates are located in @tbl-acc1b. Clear words had higher accuracy compared to high blur words, $b$ = `r a$hypothesis$Estimate`, 95% CrI \[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`. Low blurred had higher accuracy comapred to high blurred words, $b$ = `r b$hypothesis$Estimate`, 95% CrI \[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r process_ER_column(b$hypothesis$Evid.Ratio)`. The evidence was ambiguous for accuracy differences between clear and low blurred words, b = `r c$hypothesis$Estimate`, 95% CrI \[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r c$hypothesis$Evid.Ratio`.
...
fit_mem_noc <- read_rds("https://osf.io/zv5pd/download")

#get the lowblur vs. c conrtast
fit_mem_lbc <- read_rds("https://osf.io/485vx/download")
```

```{r}
#HB > C
a <- hypothesis(fit_mem_noc , "isold1:blurblur1 > 0")
b <- hypothesis(fit_mem_noc , "isold1:blurblur2 > 0")
...
The compensatory and stage-specific accounts of disfluency make contrasting predictions about the joint influence of blurring and word frequency. The **compensatory processing account** posits that perceptual difficulty increases top-down processing during word recognition, especially for low-frequency words. This enhanced support is thought to improve encoding quality and drive better memory for disfluent stimuli. Because the account is focused on lexical-level processes, it predicts an interaction between blurring and word frequency on \$\\mu\$, reflecting slower but more effortful word recognition under degraded conditions. Importantly, the compensatory view assumes little or no effect on \$\\tau\$, which is typically associated with later, extra-lexical processing. Thus, stimuli that are hard to perceive and low in frequency should recruit more lexical-level attention, resulting in improved memory specifically for blurred, low-frequency words.
...
Taken together, the stage-specific account suggests that disfluency effects on memory may depend on the degree to which processing demands extend beyond lexical access and engage post-lexical control mechanisms. If blurring and lexical difficulty interact to increase these demands—as reflected in longer response time tails (i.e., elevated \$\\tau\$)—we may observe downstream consequences for memory. However, any mnemonic benefit is expected to emerge primarily for stimuli that are otherwise processed fluently, such as high-frequency words, and only when cognitive resources remain available to support deeper encoding. The present experiment examines these possibilities by testing whether blurring and word frequency interact to influence early and late components of processing (as indexed by ex-Gaussian parameters) and whether these effects are associated with differences in subsequent memory performance.
...
To fit models to the data from Experiment 2, we constructed contrast codes for the variables Blur and Frequency using the hypr package in R (Schad et al., 2019), applying ANOVA-style (effects) coding. For the Blur variable, we defined two models that contained three orthogonal contrasts to capture key comparisons of interest. In model 1, comparison 1 contrasted High Blur and Low Blur, with High Blur coded as 0.5 and Low Blur as 0.5 (with Clear coded as 0). Comparison 2 contrasted High Blur and Clear, with High Blur coded as 0.5 and Clear as -0.5 (with Low Blur coded as 0). These contrasts allowed us to estimate both main effects and interaction terms. For the Frequency variable, we used a single contrast comparing High Frequency to Low Frequency, with High Frequency coded as 0.5 and Low Frequency as –0.5. Additionally, we fit a second model to capture the Low Blur (0.5) and Clear (-0.05) comparison. This contrast structure enabled us to extract all relevant main effects and interactions across conditions.
...
e = hypothesis(fit_acc_sc_lb, "frequency1 = 0")
...
f = hypothesis(fit_acc_sc, "blur1:frequency1 = 0")
...
g = hypothesis(fit_acc_sc , "blur2:frequency1 = 0")
...
h = hypothesis(fit_acc_sc_lb, "blur1:frequency1 = 0")

# Ensure rounding applies only to numeric columns
h$hypothesis <- h$hypothesis |>
  mutate(across(where(is.numeric), ~ round(.x, 3)))

tab <- bind_rows(a$hypothesis, b$hypothesis, c$hypothesis, d$hypothesis, e$hypothesis, f$hypothesis, g$hypothesis,h$hypothesis ) |>
    mutate(Evid.Ratio=as.numeric(Evid.Ratio))|>
   rename("Mean" = "Estimate") |>
  select(-Star)
...
acc_exp2 <- tab |> 
    mutate(Hypothesis = c("High Blur - Clear < 0", "High Blur-Low Blur < 0", "Low Blur - Clear = 0","HF - LF = 0","HF - LF = 0 (LB-C)", "(High Blur-Clear)*(HF-LF) < 0", "(High Blur-Low Blur)*(HF-LF) < 0", "(Low Blur-Clear)*(HF-LF) =  0")) |>
   mutate(
        mutate(mutate(across(where(is.numeric), ~ round(.x, 3)))),
        `90% CrI` = str_glue("[{CI.Lower}, {CI.Upper}]")) |>
    rename("SE" = "Est.Error", "ER" = "Evid.Ratio", `Posterior Prob` = "Post.Prob") |>
...
The model summary can be found in @tbl-acc2. Clear words were better identified than high-blur words, $b$ = `r a$hypothesis$Estimate`, 90% CrI \[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`. Low-blur words were also better identified than high-blur words, $b$ = `r b$hypothesis$Estimate`, 90% CrI \[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r b$hypothesis$Evid.Ratio`. There was weak evidence for a difference between clear and low-blur words, $b$ = `r c$hypothesis$Estimate`, 95% CrI \[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r c$hypothesis$Evid.Ratio`. There was no frequency effect for the High blur vs. Clear/High blur vs. Low blur model: $b$ = `r d$hypothesis$Estimate`, 95% CrI \[`r d$hypothesis$CI.Lower`, `r d$hypothesis$CI.Upper`\], ER = `r d$hypothesis$Evid.Ratio`, or the Low blur vs. clear model: $b$ = `r e$hypothesis$Estimate`, 95% CrI \[`r e$hypothesis$CI.Lower`, `r e$hypothesis$CI.Upper`\], ER = \`r e$hypothesis$Evid.Ratio although the evidence for the absence of a difference was weak.
...
Given the complexity of the model, we employed stronger priors to facilitate convergence. For the \$\\mu\$ parameter, we specified the same prior used in Experiments 1a and 1b. For the \$\\beta\$ parameter, we applied a more constrained prior: \$\\text{Normal}(0, 0.25)\$. Default priors were retained for all remaining model parameters. To ensure reliable convergence in the model examining the contrast between low blur and clear conditions and their interaction with word frequency, we increased the number of iterations to 6,000.
...
freqc <- hypr(HIGH ~ LOW, levels = c("HIGH", "LOW"))
rts_dim$frequency <- as.factor(p_rt_out$frequency)
...
rts_dim <- rts_wf %>%
  filter(corr == 1, category == "NONAN") %>%
  filter(rt >= 0.2 & rt <= 2.5)

blurC <- hypr(LB ~ C, levels = c("C", "HB", "LB"))
rts_dim$blur <- as.factor(rts_dim$blur)
contrasts(rts_dim$blur) <- contr.hypothesis(blurC)

freqc <- hypr(HIGH ~ LOW, levels = c("HIGH", "LOW"))
rts_dim$frequency <- as.factor(rts_dim$frequency)
contrasts(rts_dim$frequency) <- contr.hypothesis(freqc)
```
...
fit_sc <- read_rds("https://osf.io/3vtjq/download")
...
v = hypothesis(fit_sc_lc, "frequency1 <  0", dpar="mu")$hypothesis |> mutate(across(where(is.numeric), ~ round(.x, 3)), Parameter = "mu")
...
w = hypothesis(fit_sc_lc, "frequency1 =  0", dpar="mu")$hypothesis |> mutate(across(where(is.numeric), ~ round(.x, 3)), Parameter = "mu")
...
z = hypothesis(fit_sc_lc, "beta_frequency1 <  0", dpar="beta")$hypothesis |> mutate(across(where(is.numeric), ~ round(.x, 3)), Parameter = "beta")
...
tab <- bind_rows(a, b, c, d,v,e, f, g, h, i, j, k, l, m,w, n, o, p, q, r,z,  s, t, u) |>
  mutate(Evid.Ratio = as.numeric(Evid.Ratio)) |>
  mutate(Hypothesis = c("High Blur - Clear > 0", "High Blur-Low Blur > 0", "Low Blur - Clear = 0","HF - LF < 0 ", "HF - LF < 0 (LB-C))","(High Blur-Clear)*(HF - LF) = 0", "(High Blur-Low Blur)*(High - Low) = 0", "(Low Blur-Clear)*(HF-LF) =  0", "High Blur - Clear > 0", "High Blur-Low Blur > 0", "Low Blur - Clear = 0","HF - LF < 0", "HF - LF < 0 (LB-C))", "(High Blur-Clear)*(High - Low) = 0", "(High Blur-Low Blur)*(High-Low) = 0", "(Low Blur-Clear) - (High-Low) =  0", "High Blur - Clear > 0", "High Blur-Low Blur > 0", "Low Blur - Clear = 0","HF - LF = 0", "HF - LF < 0 (LB-C))", "(High Blur-Clear)*(HF-LF) < 0", "(High Blur-Low Blur)*(HF-Fw) < 0", "(Low Blur-Clear)*(HF-LF) =  0")) |> 
  select(-Star) |>
...
High-blurred words showed greater central tendency shifts (𝜇) compared to clear words, *b* = `r a$Estimate`, 90% CrI \[`r a$CI.Lower`, `r a$CI.Upper`\], ER = `r a$Evid.Ratio`. They also showed more shifting than low-blurred words, b = `r b$Estimate`, 90% CrI \[`r b$CI.Lower`, `r b$CI.Upper`\], ER = `r b$Evid.Ratio`. Low-blurred words were also associated with greater shifting than clear words, b = `r c$Estimate`, 90% CrI \[`r c$CI.Lower`, `r c$CI.Upper`\], ER = `r c$Evid.Ratio`.

There was a consistent effect of word frequency: high-frequency words exhibited less shifting than low-frequency words, b = `r d$Estimate`, 90% CrI \[`r d$CI.Lower`, `r d$CI.Upper`\], ER = `r d$Evid.Ratio`. This pattern also appeared in the low-blurred vs. clear contrast, b = `r v$Estimate`, 90% CrI \[`r v$CI.Lower`, `r v$CI.Upper`\], ER = `r v$Evid.Ratio`.

The posterior distributions for blur × frequency interactions were centered near zero, and the associated evidence ratios provided little support for interaction effects, suggesting that blur and frequency contributed additively to shifts in central tendency.

### Variability (𝜎)

There was evidence for greater variability (𝜎) in response times for high-blurred relative to clear words, b = `r h$Estimate`, 90% CrI \[`r h$CI.Lower`, `r h$CI.Upper`\], ER = `r h$Evid.Ratio`. A similar trend was observed for low-blurred compared to clear words, b = `r i$Estimate`, 90% CrI \[`r i$CI.Lower`, `r i$CI.Upper`\], ER = `r i$Evid.Ratio`. However, the contrast between low-blurred and clear words yielded only weak evidence, b = `r j$Estimate`, 90% CrI \[`r j$CI.Lower`, `r j$CI.Upper`\], ER = `r j$Evid.Ratio`.

There was little to no evidence for a frequency effect on variability, b = `r k$Estimate`, 90% CrI \[`r k$CI.Lower`, `r k$CI.Upper`\], ER = `r k$Evid.Ratio`, which was also the case in the low-blurred vs. clear contrast, b = `r w$Estimate`, 90% CrI \[`r w$CI.Lower`, `r w$CI.Upper`\], ER = `r w$Evid.Ratio`.

Evidence for blur × frequency interactions on 𝜎 was mixed, with some contrasts yielding ERs modestly greater than 1, but posterior distributions remained broadly overlapping with zero.

### Skew (𝛽)

Posterior estimates indicated greater skew (𝛽) for high-blurred words relative to clear words, b = `r o$Estimate`, 90% CrI \[`r o$CI.Lower`, `r o$CI.Upper`\], ER = `r o$Evid.Ratio`, and also relative to low-blurred words, b = `r p$Estimate`, 90% CrI \[`r p$CI.Lower`, `r p$CI.Upper`\], ER = `r p$Evid.Ratio`. In contrast, the difference in skew between low-blurred and clear words was minimal, with strong evidence for no effect, b = `r q$Estimate`, 95% CrI \[`r q$CI.Lower`, `r q$CI.Upper`\], ER = `r q$Evid.Ratio`.

There was strong evidence that low-frequency words showed more rightward skew than high-frequency words in both the high-blurred vs. low-blurred and high-blurred vs. clear contrasts, b = `r r$Estimate`, 95% CrI \[`r r$CI.Lower`, `r r$CI.Upper`\], ER = `r r$Evid.Ratio`. A similar frequency effect was present in the low-blurred vs. clear contrast, b = `r z$Estimate`, 90% CrI \[`r z$CI.Lower`, `r z$CI.Upper`\], ER = `r w$Evid.Ratio`.

The interaction between blur and frequency on 𝛽 was supported in the high-blurred vs. low-blurred comparison, b = `r s$Estimate`, 90% CrI \[`r s$CI.Lower`, `r s$CI.Upper`\], ER = `r s$Evid.Ratio`, where the frequency effect on skew appeared more pronounced. However, no meaningful interaction was detected in the high-blurred vs. clear or low-blurred vs. clear contrasts, b = `r t$Estimate`, 90% CrI \[`r t$CI.Lower`, `r t$CI.Upper`\], ER = `r t$Evid.Ratio`, and b = `r u$Estimate`, 95% CrI \[`r u$CI.Lower`, `r u$CI.Upper`\], ER = `r u$Evid.Ratio`.
...
fit_sc_mem <- read_rds("https://osf.io/5wgpn/download")

fit_sc_mem_lb <- read_rds("https://osf.io/fc4tg/download")
...
Experiment 2 probed how late‐stage processing underlies the mnemonic boost that disfluent stimuli sometimes confer—and how that boost interacts with word frequency. Looking at accuracy, we did not see any differecens between the levels of blurring, or any interactions with frequency. We did see an accuracy difference between high frequency and low frequency words. This largely corresponds with what @fernández-lópez2022 found.

Our ex-Gaussian analysis revealed a clear dissociation between two critical contrasts. When comparing high blur to low blur, $\beta$ increased significantly more for low-frequency words than for high-frequency words. This pattern suggests that low-frequency items, already more vulnerable under mild blur, were disproportionately affected by additional perceptual noise—resulting in longer, late-stage processing delays. In contrast, the transition from clear to high blur produced a substantial and uniform increase in τ across word frequency. High blur impacted all words equally. This pattern can be clearly seen in Figure X.

Turning to memory, both blurred conditions showed a perceptual disfluency effect relative to their respective baselines (perceptual disfluency effect). Crucially, however, only the clear→high-blur contrast showed a frequency-specific memory gain: high-frequency words gained substantially from heavy blur, whereas low-frequency words did not. In the low-to-high blur comparison, memory for high- and low-frequency words improved equally, despite the fact that low-frequency items suffered larger τ increases.

This pattern—non-additivity in τ for the high-vs-low blur contrast, but a frequency-gated memory benefit only in the clear-vs-high blur contrast—suggests that not all extra processing time is equally mnemonic. High-frequency words under heavy blur can translate occasional “hang-ups” into strategic, elaborative repair: their robust lexical representations survive the noise long enough to trigger deeper encoding. Low-frequency words, in contrast, spend disproportionate τ‐driven time on unproductive perceptual guessing under both mild and severe blur, yielding no selective memory benefit. Taken together, these findings reveal a “sweet-spot” of disfluency. A calibrated step from low to high blur unmasks how baseline difficulty shapes late-stage control engagement, but that engagement alone does not guarantee stronger memory. Only when extreme blur uniformly taxes perception—yet leaves intact the strong lexical substrates of familiar words—can disfluency reliably boost encoding.
