...
For all models (except the diffusion decision models), we applied ANOVA-style (effects) coding using contrast variables. For the Blur factor in Experiments 1a, 1b, and 2, we defined two orthogonal contrasts to capture the primary comparisons of interest.

-   Contrast 1 compared High Blur against the average of Clear and Low Blur. Here, High Blur was coded as 0.5, while both Clear and Low Blur were coded as -0.5.
-   Contrast 2 isolated the difference between Low Blur (coded as 0.5) and Clear (coded as -0.5), with High Blur coded as 0.

In Experiment 2, we also included a Frequency factor, where High Frequency was coded as 0.5 and Low Frequency as -0.5.
...
fit_sc <- read_rds(here::here("_manuscript", "models", "fit_sc_exg.rds"))
...
tab <- bind_rows(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o) |>
  mutate(
    ...
    `90% CrI` = str_glue("[{CI.Lower}, {CI.Upper}]"),
    Hypothesis = c(
      "High Blur (vs. Clear/Low Blur) > 0",
      "Low Blur - Clear > 0",
      "High Frequency - Low Frequency < 0",
      "High Blur (vs. Low Blur/Clear) × Frequency  = 0 ",
      "Low Blur (vs. Clear) × Frequency = 0",
      
      "High Blur (vs. Clear/Low Blur) > 0",
      "Low Blur - Clear < 0",
      "High Frequency  - Low Frequency < 0",
      "High Blur (vs. Low Blur/Clear) = 0",
      "Low Blur (vs. Clear) × Frequency = 0",
...
and for the low blur (vs. clear) × grequency interaction:\
...
In contrast, the low blur (vs. clear) × fequency interaction showed strong evidence for greater skewing for high frequency words (even though CrI included 0), \
...
ggsave("dprime_expt2.png", width=12, height=8, dpi=500)
