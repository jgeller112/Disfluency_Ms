...
#set contrasts in df 
blur_rt_new$blur <- as.factor(blur_rt$blur)

contrasts(blur_rt_new$blur) <-contr.hypothesis(blurC)
...
fit_c <- read_rds("https://osf.io/9jr4z/download")
...
tmp |>   
  ...
  stat_halfeye(colour="black", 
               .width = c(0.66, 0.90), point_interval = "mean_qi") + 
...
fit_exg1 <- read_rds("https://osf.io/9jr4z/download")
...
tmp |>   
  ...
  stat_halfeye(colour="black", .width = c(0.66, 0.90), point_interval = "mean_qi") + 
    facet_grid(Parameter~t, scales = "free") + 
    geom_hline(yintercept = 0, linewidth = .25) + 
  theme_minimal(base_size=16)
