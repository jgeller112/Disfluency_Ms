...
In Experiment 2, we chose not to use the DDM due to its substantial computational demands, particularly given the size and complexity of the current dataset. Instead, we focused on the ex-Gaussian decomposition of RTs during encoding. Findings from Experiments 1a and 1b suggest a relationship between the ex-Guass and DDM parameters, insofar there was a positive association  between $\mu$ and $T_{er}$ and a negative association between $\beta$ and $v$. Given this, we use these parameters as an indirect marker of early and late processes. 
...
acc_exp2 <- tab |> 
    mutate(Hypothesis = c("High Blur - Clear < 0", "High Blur-Low Blur < 0", "Low Blur - Clear = 0","High - Low ",  "(High Blur-Clear)*(High-Low) < 0", "(High Blur-Low Blur)* ((High-Low) < 0", "(Low Blur-Clear)*(High-Low) =  0")) |>
   mutate(
        mutate(mutate(across(where(is.numeric), ~ round(.x, 3)))),
        `90% CrI` = str_glue("[{CI.Lower}, {CI.Upper}]")) |>
    rename("SE" = "Est.Error", "ER" = "Evid.Ratio", `Posterior Prob` = "Post.Prob") |>
