...
The compensatory and stage-specific accounts of disfluency make contrasting predictions about the joint influence of blurring and word frequency. The compensatory processing account posits that perceptual difficulty elicits increased top-down support during lexical access, particularly for low-frequency words, and this is what causes a mnmnmomic benefit for disfluent stimuli. This account would likely predict an interaction between blurring and word frequency on $\mu$, reflecting slowed encoding under high-effort conditions. It assumes little or no effect on $\tau$. Stimuli that are lower in frequency and hard to read should receive increased attention during encoding. Consequently, the compensatory view predicts better memory for blurred, low-frequency words due to greater top-down engagement during encoding.

In contrast, the stage-specific account posits that the effects of blurring and word frequency extend beyond encoding and into later, resource-demanding stages of processing. This account would likely predict interactions between blurring and frequency on $\tau$, as more difficult stimuli impose greater demands on post-lexical mechanisms such as cognitive control and decision-making.
...
Critically, if an interaction emerges on $\tau$—and we do observe a memory benefit—this would support the stage-specific account. Such a pattern would indicate that disfluency effects are driven not by compensatory processing, but by increased demands on limited-capacity control processes engaged after lexical access.
...
#| eval: false
#|
prior_exp1 <- c(set_prior("normal(0,10)", class = "b", coef=""))
...
High-blurred words exhibited greater shifting than clear words, $b$ = `r a$Estimate`, 90% CrI \[`r a$CI.Lower`, `r a$CI.Upper`\], ER = `r a$Evid.Ratio`; as did low-blurred words, $b$ = `r b$Estimate`, 90% CrI \[`r b$CI.Lower`, `r b$CI.Upper`\], ER = `r b$Evid.Ratio`. However, there was no evidence of a difference between low-blurred and clear words, $b$ = `r c$Estimate`, 90% CrI \[`r c$CI.Lower`, `r c$CI.Upper`\], ER = `r c$Evid.Ratio`.

For word frequency, high-frequency words showed less shifting than low-frequency words, $b$ = `r d$Estimate`, 90% CrI \[`r d$CI.Lower`, `r d$CI.Upper`\], ER = `r d$Evid.Ratio`. This effect also appeared in the low-blurred vs. clear model, $b$ = `r v$Estimate`, 90% CrI \[`r v$CI.Lower`, `r v$CI.Upper`\], ER = `r v$Evid.Ratio`.

There was no interaction between word frequency and blur for high-blurred vs. clear words, $b$ = `r e$Estimate`, 90% CrI \[`r e$CI.Lower`, `r e$CI.Upper`\], ER = `r e$Evid.Ratio`, or low-blurred vs. clear words, $b$ = `r f$Estimate`, 95% CrI \[`r f$CI.Lower`, `r f$CI.Upper`\], ER = `r f$Evid.Ratio`. Likewise, there was no amplification of the word frequency effect in the low-blur vs. clear comparison, $b$ = `r g$Estimate`, 95% CrI \[`r g$CI.Lower`, `r g$CI.Upper`\], ER = `r g$Evid.Ratio`.

### $\sigma$

High-blurred words exhibited greater variance than clear words, $b$ = `r h$Estimate`, 90% CrI \[`r h$CI.Lower`, `r h$CI.Upper`\], ER = `r h$Evid.Ratio`, as did low-blurred words, $b$ = `r i$Estimate`, 90% CrI \[`r i$CI.Lower`, `r i$CI.Upper`\], ER = `r i$Evid.Ratio`. Weak evidence was found for greater variance in low-blurred vs. clear words, $b$ = `r j$Estimate`, 90% CrI \[`r j$CI.Lower`, `r j$CI.Upper`\], ER = `r j$Evid.Ratio`.

No difference in $\sigma$ was observed between high- and low-frequency words, $b$ = `r k$Estimate`, 90% CrI \[`r k$CI.Lower`, `r k$CI.Upper`\], ER = `r k$Evid.Ratio`, which also held in the low-blurred vs. clear model, $b$ = `r w$Estimate`, 90% CrI \[`r w$CI.Lower`, `r w$CI.Upper`\], ER = `r w$Evid.Ratio`.

Evidence for blur × frequency interactions on $\sigma$ was mixed (ER \> 1).

### $\beta$

High-blurred words showed greater skew than clear words, $b$ = `r o$Estimate`, 90% CrI \[`r o$CI.Lower`, `r o$CI.Upper`\], ER = `r o$Evid.Ratio`, as did low-blurred words, $b$ = `r p$Estimate`, 90% CrI \[`r p$CI.Lower`, `r p$CI.Upper`\], ER = `r p$Evid.Ratio`. There was strong evidence for no difference in skew between low-blurred and clear words, $b$ = `r q$Estimate`, 95% CrI \[`r q$CI.Lower`, `r q$CI.Upper`\], ER = `r q$Evid.Ratio`.

There was no evidence that low-frequency words showed greater skew than high-frequency words, $b$ = `r r$Estimate`, 95% CrI \[`r r$CI.Lower`, `r r$CI.Upper`\], ER = `r r$Evid.Ratio`. However, in the low-blurred vs. clear model, low-frequency words did show greater skewing, $b$ = `r z$Estimate`, 90% CrI \[`r z$CI.Lower`, `r z$CI.Upper`\], ER = `r w$Evid.Ratio`.

The word frequency effect on skew was amplified for the high-blurred vs. clear words, $b$ = `r s$Estimate`, 90% CrI \[`r s$CI.Lower`, `r s$CI.Upper`\], ER = `r s$Evid.Ratio`, and low-blurred vs. clear words, $b$ = `r t$Estimate`, 90% CrI \[`r t$CI.Lower`, `r t$CI.Upper`\], ER = `r t$Evid.Ratio`, with greater skewing for low- than high-frequency words. No interaction was found in the low-blurred vs. clear contrast, $b$ = `r u$Estimate`, 95% CrI \[`r u$CI.Lower`, `r u$CI.Upper`\], ER = `r u$Evid.Ratio`.
