...
Accuracy (coded as correct vs. incorrect) was modeled using a Bayesian logistic regression with a Bernoulli distribution. We specified a Cauchy(0, 0.35) prior for population-level effects, as used in @kinoshita2023.
...
Recognition memory data were analyzed using a Bayesian generalized linear multilevel model (Bernoulli distribution with a probit link). Following Signal Detection Theory (SDT; [@green1966]), correct responses can either be participant responses ("old" vs. "new") were modeled as a function of item status ("old" vs. "new") and blurring level. We specified a Cauchy(0, 0.35) prior for population-level effects.
