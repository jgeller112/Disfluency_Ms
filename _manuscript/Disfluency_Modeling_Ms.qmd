...
ft <- flextable(data) %>%
  ...
  merge_v(j = c("Account", "Description", "Loci")) %>%  # Merge duplicate cells for readability
  align(j = c("Account", "Description", "Loci", "Contrast"), align = "left", part = "body") %>%
  align(j = c("ExGaussianPredictions", "DDMPredictions", "QuantilePlots", "RecognitionMemoryPredictions"), align = "center", part = "body") %>%
  bold(part = "header") %>%
  autofit()
...
acc_exp1a <- tab %>% 
    ...
    select(Hypothesis, Estimate, SE, `90% CrI`,ER, Post.Prob)%>% 
  flextable() %>%
  theme_apa()
