...
# Create data frame with updated recognition memory predictions
data <- data.frame(
    ...
    RecognitionMemoryPredictions = c(
        "High > Low/Clear", "Low > Clear",      # Meta-cognitive (updated)
        "High > Low/Clear", "×",               # Compensatory-processing
        "High > Low/Clear", "×"                # Stage-specific
    ),
...
# Create flextable with symbol note
ft <- flextable(data) |>
  set_header_labels(
    Account = "Account",
  ...
  bold(part = "header") |>
  set_caption("Note: ↑ = Increase, × = No difference, ↓ = Decrease")
...
a <- hypothesis(fit_c, "blurHB - ((blurLB + blurC)/2 > 0)", dpar = "mu")
b <- hypothesis(fit_c, "blurLB - blurC > 0", dpar = "mu")

c <- hypothesis(fit_c, "sigma_blurHB - ((sigma_blurLB + sigma_blurC)/2) > 0", dpar = "sigma")
d <- hypothesis(fit_c, "sigma_blurLB - sigma_blurC = 0", dpar = "sigma")

e <- hypothesis(fit_c, "beta_blurHB - ((beta_blurLB + beta_blurC)/2 > 0)", dpar = "beta")
f <- hypothesis(fit_c, "beta_blurLB - beta_blurC = 0", dpar = "beta")
...
a <- hypothesis(fit_wiener, "blurHB - ((blurLB + blurC) / 2)", dpar = "mu")
b <- hypothesis(fit_wiener, "blurLB - blurC > 0", dpar = "mu")
...
A summary of the diffusion model results can be found in @tbl-diffexpt1a. High-blurred words had a lower drift rate than clear and low blurred words, *b* = `r a$hypothesis$Estimate`, 90% CrI \[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`. There was no difference in drift rate between low-blurred words and clear words, *b* = `r b$hypothesis$Estimate`, 95% CrI \[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r process_ER_column(b$hypothesis$Evid.Ratio)`.

Non-decision time was higher for high-blurred words compared to clear and low blurred words, *b* = `r c$hypothesis$Estimate`, 90% CrI \[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r process_ER_column(c$hypothesis$Evid.Ratio)`. Low-blurred words had a higher non-decision time than clear words, *b* = `r d$hypothesis$Estimate`, 90% CrI \[`r d$hypothesis$CI.Lower`, `r d$hypothesis$CI.Upper`\], ER = `r process_ER_column(d$hypothesis$Evid.Ratio)`.
...
# Define contrasts using averages and differences
a <- hypothesis(fit_wiener, "blurHB - (blurLB + blurC)/2 < 0", dpar = "mu")
b <- hypothesis(fit_wiener, "blurLB - blurC < 0", dpar = "mu")
...
A summary of the diffusion model results can be found in @tbl-diffexpt1b. High-blurred words had a lower drift rate than clear and low blurred words, *b* = `r a$hypothesis$Estimate`, 90% CrI \[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r process_ER_column(a$hypothesis$Evid.Ratio)`. There was no difference in drift rate between low-blurred words and clear words, *b* = `r b$hypothesis$Estimate`, 90% CrI \[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r process_ER_column(b$hypothesis$Evid.Ratio)`.
...
fit_acc_sc <- read_rds("https://osf.io/uz3jc/download")
...
#| fig-cap: Estimated posterior distributions for d-prime and criterion, and differences between all conditions,  95% CrIs (thin lines)
#| fig-width: 12
#| fig-height: 10
...
Similar to Experiments 1a and 1a, there was better memory recognition for high blurred words compared to clear words and low blurred words (see @fig-m2-emmeans), $b$ = `r a$hypothesis$Estimate`, 90% CrI\[`r a$hypothesis$CI.Lower`, `r a$hypothesis$CI.Upper`\], ER = `r a$hypothesis$Evid.Ratio`.  There was no recognition memory difference between clear and low blur words, $\beta$ = `r b$hypothesis$Estimate`, 95% CrI\[`r b$hypothesis$CI.Lower`, `r b$hypothesis$CI.Upper`\], ER = `r b$hypothesis$Evid.Ratio`. Low frequency words were better recognized than high frequency words, $\beta$ = `r c$hypothesis$Estimate`, 90% CrI\[`r c$hypothesis$CI.Lower`, `r c$hypothesis$CI.Upper`\], ER = `r c$hypothesis$Evid.Ratio`. There was strong evidence for an interaction between high blurred words (vs. low blur/clear) and frequency, suggesting that the memory benefit was selectively enhanced for high-frequency words under conditions of high blur, $\beta$ = `r d$hypothesis$Estimate`, 95% CrI\[`r d$hypothesis$CI.Lower`, `r d$hypothesis$CI.Upper`\], ER = `r d$hypothesis$Evid.Ratio`. There was also strong evidence for an interaction between blurring and frequency for high low blurred words vs. clear words, $\beta$ = `r e$hypothesis$Estimate`, 90% CrI\[`r e$hypothesis$CI.Lower`, `r e$hypothesis$CI.Upper`\], ER = `r e$hypothesis$Evid.Ratio`, with clear words showing improved recognition for low-frequency items relative to high-frequency ones.
